...
function abrirDB() {
  ...
  return new Promise((resolve, reject) => {
    ...
    solicitude.onsuccess = () => {
      resolve(solicitude.result);
    };

    // Xestión de Errores
    solicitude.onerror = () => {
      reject(solicitude.error);
    };
  });
}
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  abrirDB()
    ...
    .catch((erro) => {
      console.error(
        "[IndexedDB] Erro ao abrir a base de datos:",
        solicitude.error
      );
  ...
  document
    .getElementById("nome-evento")
    .addEventListener("keypress", (tecla) => {
      if (tecla.key === "Enter") {
        const btnEngadir = document.getElementById("btn-engadir");
